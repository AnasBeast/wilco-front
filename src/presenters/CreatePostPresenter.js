...
import { FLIGHT, MESSAGE, TITLE, VISIBILITY } from '../constants/formFields/postForm';
...
export default class CreatePostPresenter {
  ...
  onAddContributorsPressed = () => {
	this.navigation.navigate(AUTHENTICATED_ROUTES.Contributors.name);
  }

  onSelectCommunitiesPressed = () => {
	this.navigation.navigate(AUTHENTICATED_ROUTES.Communities.name);
  }
  ...
  onAircraftFlightSelected(flightData) {
    const { flightParams, aircraft, hasSelectedFlight, hasManualFlight } = flightData;
  ...
  async onNewPost({ title, message, visibility, communityTags, flightParams, base64Photos }) {
  ...
  visibilityInputWasPressed = () => {
    this.modalService.open(VISIBILITY_PICKER_MODAL, {
  ...
  get _selectedVisibility() {
    return this.visibilityOptions.find((visibility) => visibility.value === this._selectedVisibilityId);
  }
  ...
  get _wasAnythingFilled() {
    ...
    return (
      title || message || this.isAnyPhotoSelected || this.flightParams != null || this.communityTagsPresenter.hasAnyTag
    );
  }
  ...
  get _hasNeitherTextFlightNorPhotos() {
    return (
      !this.form.$(MESSAGE).value && !this.form.$(TITLE).value && !this.isAnyPhotoSelected && this.flightParams === null
    );
  }
}